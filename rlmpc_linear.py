...
class drone_mpc_env(gym.Env):
    def __init__(self):
        ...
        self.save_path = "C:/Users/kevin/Desktop/MEng_Project/Distance_Reward/"
        ...
        self.action_space = gymnasium.spaces.box.Box(low=-1, high=1, shape=(24+self.m_act_len,), dtype=np.float32)
        ...
        self.action_space_lower = np.hstack([7*np.ones(2,),7*np.ones(1,),0.5*np.ones(7,),0.00001*np.ones(1,),0.5*np.ones(3,),7*np.ones(2,),7*np.ones(1,),0.5*np.ones(7,),
                                             -60*np.ones(8,),-120*np.ones(2,),-0.5*np.ones(12,),-4*np.ones(2,),-3*np.ones(1,),-0.5*np.ones(12,)]).astype(np.float32)
        self.action_space_higher = np.hstack([9*np.ones(2,),9*np.ones(1,),1.5*np.ones(7,),0.0002*np.ones(1,),1.5*np.ones(3,),9*np.ones(2,),9*np.ones(1,),1.5*np.ones(7,),
                                             60*np.ones(8,),120*np.ones(2,),0.5*np.ones(12,),4*np.ones(2,),1*np.ones(1,),0.5*np.ones(12,)]).astype(np.float32)
    ...
    def linearize(self):
        ...
        self.A_dict = { (3,6): (-60,60), (3,7): (-60,60), (3,8): (-60,60), (3,9): (-60,60), 
                  (4,6): (-60,60), (4,7): (-60,60), (4,8): (-60,60), (4,9): (-60,60),
                  (5,7): (-120,120), (5,8): (-120,120), (6,7): (-0.5,0.5), (6,8): (-0.5,0.5), (6,9): (-0.5,0.5),
                  (7,6): (-0.5,0.5), (7,8): (-0.5,0.5), (7,9): (-0.5,0.5), (8,6): (-0.5,0.5), (8,7): (-0.5,0.5), (8,9): (-0.5,0.5),
                  (9,6): (-0.5,0.5), (9,7): (-0.5,0.5), (9,8): (-0.5,0.5)}
        
        self.B_dict = {(3,0): (-4,4), (4,0): (-4,4), (5,0): (-3,1), (6,1): (-0.5,0.5), (6,2): (-0.5,0.5), (6,3): (-0.5,0.5), 
                       (7,1): (-0.5,0.5), (7,2): (-0.5,0.5), (7,3): (-0.5,0.5), (8,1): (-0.5,0.5), (8,2): (-0.5,0.5), (8,3): (-0.5,0.5),
                       (9,1): (-0.5,0.5), (9,2): (-0.5,0.5), (9,3): (-0.5,0.5)}
        
        self.m_act_len = len(self.A_dict) + len(self.B_dict)

        self.A_keys = list(self.A_dict.keys())
        self.B_keys = list(self.B_dict.keys())
    ...
    def mpc(self, x0, x_ref):
        ...
        # Functions
        x = ca.SX.sym('x', self.X_dim)
        u = ca.SX.sym('u', self.U_dim)
        A_d = np.eye(self.X_dim) + self.A*self.t
        B_d = self.B*self.t
        x_next = A_d@x + B_d@u

        cost_expr = (x - x_ref).T@self.Q@(x - x_ref) + u.T@self.R@u
        f_dynamics = ca.Function('f_dynamics', [x,u], [x_next]) 
        f_cost = ca.Function('f_cost', [x,u], [cost_expr])
        ...
        cost += ca.mtimes([(X[:, self.N] - x_ref).T, self.Q_bar, X[:, self.N] - x_ref]) # cost for terminal state
    ...
    def step(self,action):
        ...
        self.Q_bar = np.diag(self.unnormalize(action[14:24],self.action_space_lower[14:24],self.action_space_higher[14:24]))
        
        A_end = 24+len(self.A_dict)
        self.A_vals = self.unnormalize(action[24:A_end],self.action_space_lower[24:A_end],self.action_space_higher[24:A_end])
        self.B_vals = self.unnormalize(action[A_end:],self.action_space_lower[A_end:],self.action_space_higher[A_end:])
        ...
        for idx in range(self.A_vals.shape[0]):
            self.A[self.A_keys[idx][0],self.A_keys[idx][1]] = self.A_vals[idx]
        
        for idx in range(self.B_vals.shape[0]):
            self.B[self.B_keys[idx][0],self.B_keys[idx][1]] = self.B_vals[idx]
        ...
        if cost == 1000000:
            reward = -3*((10/self.t) - (self.time/self.t))
            terminated = True
            truncated = False
        else:
            reward = self.normalized_distance_reward(self.state,self.reference)
            terminated = False
...
#                 self.writer.add_scalar('total_loss', total_loss, self.num_timesteps)
...
#             total_loss = actor_loss + critic_loss
#             self.tb_formatter.writer.add_scalar('train/total_loss', total_loss, self.num_timesteps)
...
def train(env):
    model = SAC('MlpPolicy', env, tensorboard_log='C:/Users/kevin/Desktop/MEng_Project/tensorboard_logs', verbose=1, seed=1)
    print(model.policy)
