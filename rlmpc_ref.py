...
class drone_mpc_env(gym.Env):
    ...
    def step(self,action):
        ...
        t_step = int(self.time/self.t)

        # Run MPC and calculate cost and control input
        cost, control_input = self.mpc(self.state,self.X_r[t_step:int(t_step+self.N+1),:])
        ...
        if cost == 1000000:
            ...
            terminated = False
            truncated = self.time >= (max(self.X_r.shape) - self.N)*self.t
